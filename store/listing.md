# ChainPlay — Google Play 스토어 등록 정보

---

## 앱 이름
ChainPlay

---

## 짧은 설명 (80자 이내)
동영상 알고리즘 순서가 아닌 내가 원하는 플레이리스트를 만들고 순차적으로 자동 재생.

---

## 전체 설명 (4000자 이내)

영상을 알고리즘이 아닌 나만의 플레이리스트로 묶어 순서대로 자동 재생하세요.

**주요 기능**

• URL 붙여넣기만으로 영상 추가 — youtube.com, youtu.be, Shorts 모두 지원
• 자동 순차 재생 — 한 영상이 끝나면 다음 영상으로 자동 이동
• 이전 / 다음 버튼으로 빠른 탐색
• 플레이리스트 순서 자유롭게 변경
• 목록 자동 저장 — 앱을 껐다 켜도 그대로
• 영상 제목과 썸네일 자동 표시

**이런 분께 추천**

- 같은 유튜브 영상을 반복해서 보시는 분
- 특정 영상들을 정해진 순서로 이어 보고 싶은 분
- 유튜브 앱의 복잡한 기능 없이 간단한 재생기가 필요한 분

**지원 URL 형식**

- youtube.com/watch?v=...
- youtu.be/...
- youtube.com/shorts/...
- youtube.com/embed/...

이 앱은 개인 사용 목적으로 만들어진 간단한 플레이리스트 플레이어입니다.
재생은 YouTube의 공식 플레이어를 통해 이루어지며, 별도의 계정 로그인이 필요 없습니다.

---

## 카테고리
동영상 플레이어 / 편집기

## 콘텐츠 등급
전체 이용가

## 태그 (키워드)
유튜브, 플레이리스트, 동영상, 자동재생, YouTube, playlist, video player

---

## 스크린샷 안내

Google Play는 폰 스크린샷 최소 2장이 필요합니다.
권장 해상도: 1080×1920 (9:16 세로)

촬영 화면 제안:
1. 플레이리스트에 영상이 3~4개 추가된 상태 + 플레이어 재생 중
2. URL 추가 모달이 열린 상태

---

## 개인정보처리방침 URL

https://hyunseokyu1-netizen.github.io/chain-play-privacy/

---
---

# ChainPlay — Google Play Store Listing (English)

---

## App Name
ChainPlay

---

## Short Description (max 80 chars)
Skip the algorithm — build your own playlist and auto-play videos in your order.

---

## Full Description

Watch videos your way — not the algorithm's way. Build your own playlist and let them play in sequence, automatically.

**Key Features**

• Add videos by pasting any YouTube URL — supports youtube.com, youtu.be, and Shorts
• Auto-play next — when one video ends, the next one starts automatically
• Previous / Next buttons for quick navigation
• Reorder your playlist freely
• Playlist saved automatically — picks up right where you left off
• Video title and thumbnail loaded automatically

**Perfect for**

- Rewatching the same YouTube videos on repeat
- Playing specific videos in a fixed order, your way
- Anyone who wants a simple player without YouTube's clutter

**Supported URL Formats**

- youtube.com/watch?v=...
- youtu.be/...
- youtube.com/shorts/...
- youtube.com/embed/...

This is a simple playlist player built for personal use. Playback is handled entirely through YouTube's official player — no account login required.

---

## Category
Video Players & Editors

## Content Rating
Everyone

## Tags / Keywords
YouTube, playlist, video player, auto-play, queue, youtube player

---

## Screenshot Guide

Google Play requires at least 2 phone screenshots.
Recommended resolution: 1080×1920 (9:16 portrait)

Suggested screens:
1. Playlist with 3–4 videos added, player actively playing
2. Add URL modal open

---

## Privacy Policy URL

https://hyunseokyu1-netizen.github.io/chain-play-privacy/



## 구글스토어
### 출시 노트

<en-US>
Watch videos your way — build your own playlist and auto-play in sequence.
</en-US>
<ko-KR>
유튜브 영상을 나만의 플레이리스트로 묶어 순서대로 자동 재생하세요.
</ko-KR>